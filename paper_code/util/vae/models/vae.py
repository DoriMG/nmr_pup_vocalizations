...
import os
from vae.plotting.grid_plot import grid_plot
...
class VAE(nn.Module):
	...
	def __init__(self, save_dir='', model_name='vae', lr=1e-3, z_dim=32, model_precision=10.0):
		r"""
		Initialize the VAE.
	...
	def forward(self, x, return_latent_rec=False):
		...
		try:
			...
			print("before multivariate",mu, u, d)
			print("x:",x,"x nan:",torch.sum(torch.isnan(mu)))
		...
		#u=u.squeeze()
		#print("after multivariate",mu.shape, u.shape, d.shape)
		#kl_divergence = -0.5 * torch.sum(1 + u - torch.pow(mu,2) -  torch.exp(u)) # added kl instead of elbo
		...
		elbo =  elbo + pxz_term #- kl_divergence # added kl took out elbo +
	...
	def train_loop(self, train_dataloader, test_dataloader, epochs=100, test_freq=2, save_freq=1, vis_freq=1):
		r"""
		...
		save_freq : int, optional
			The model is saved every `save_freq` epochs. Defaults to ``None``.
		...
		print("="*40)
		save_loss = np.zeros((2,epochs))

		for epoch in range(self.epoch, self.epoch+epochs):
			# Train
			train_loss = self.train_epoch(train_dataloader)
			self.loss['train'][epoch] = train_loss
			save_loss[0,epoch] = train_loss 
			
			# Test
			if (test_freq is not None) and (epoch % test_freq == 0):
				test_loss = self.test_epoch(test_dataloader)
				self.loss['test'][epoch] = test_loss
				save_loss[1,epoch]=test_loss
				
			# Save checkpoint
			if (save_freq is not None) and (epoch % save_freq == 0) and (epoch > 0):
				filename = f"{self.model_name}_checkpoint_{epoch}.pt"
				self.save_state(filename)
			# # Plot reconstructions.
			if (vis_freq is not None) and (epoch % vis_freq == 0):
				self.visualize(test_dataloader)

		print("="*40)
		# save final model
		filename = self.save_dir / f"{self.model_name}_final.pt"
		self.save_state(filename)
	...
	def load_state(self, filename):
		"""Load all the model parameters from the given ``.pt`` file."""
		filename = Path(filename)
		self.load_state_dict(torch.load(filename, map_location=self.device, weights_only=True))


	def visualize(self, loader, num_specs=5, gap=(2,6), save_filename='reconstruction.pdf'):
		r"""
		...
		specs =np.full([len(indices),1, 128, 128], np.nan)
		for i, idx in enumerate(indices):
			specs[i, :,:,:] = loader.dataset[idx]
			
		specs = torch.from_numpy(specs.astype(np.float32))
		# Get resonstructions.
		with torch.no_grad():
			_, _, rec_specs = self.forward(specs, return_latent_rec=True)
		specs = specs.detach().cpu().numpy()
		all_specs = np.stack([np.squeeze(specs), rec_specs])
		# Plot.
